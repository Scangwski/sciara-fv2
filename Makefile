...
ALL_CPPS = $(wildcard cpu/*.cpp)
CPP_SOURCES_FOR_CUDA = $(filter-out cpu/sciara_fv2.cpp, $(ALL_CPPS))
...
serial:
	$(NVCC) $(NVFLAGS) $(ALL_CPPS) -o $(EXEC_SERIAL)

omp:
	$(NVCC) $(NVFLAGS) -Xcompiler -fopenmp $(ALL_CPPS) -o $(EXEC_OMP)

cuda:
	$(NVCC) $(NVFLAGS) $(CPP_SOURCES_FOR_CUDA) sciara_fv2.cu -o $(EXEC_CUDA)

cuda_tiled:
	$(NVCC) $(NVFLAGS) $(CPP_SOURCES_FOR_CUDA) sciara_fv2_tiled.cu -o $(EXEC_CUDA_TILED)

cuda_tiled_halo:
	$(NVCC) $(NVFLAGS) $(CPP_SOURCES_FOR_CUDA) sciara_fv2_tiled_halo.cu -o $(EXEC_CUDA_TILED_HALO)

cuda_cfame:
	$(NVCC) $(NVFLAGS) $(CPP_SOURCES_FOR_CUDA) sciara_fv2_cfame.cu -o $(EXEC_CUDA_CFAME)
