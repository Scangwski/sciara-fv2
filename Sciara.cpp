...
void allocateSubstates(Sciara *sciara)
{
    ...
    // Substates principali (Unified Memory)
    cudaMallocManaged(&sciara->substates->Sz,      size * sizeof(double));
    cudaMallocManaged(&sciara->substates->Sz_next, size * sizeof(double));

    cudaMallocManaged(&sciara->substates->Sh,      size * sizeof(double));
    cudaMallocManaged(&sciara->substates->Sh_next, size * sizeof(double));

    cudaMallocManaged(&sciara->substates->ST,      size * sizeof(double));
    cudaMallocManaged(&sciara->substates->ST_next, size * sizeof(double));

    // Outflows buffer (8 layer)
    cudaMallocManaged(&sciara->substates->Mf,
                      size * NUMBER_OF_OUTFLOWS * sizeof(double));

    // Boundary mask e solidified lava
    cudaMallocManaged(&sciara->substates->Mb,  size * sizeof(bool));
    cudaMallocManaged(&sciara->substates->Mhs, size * sizeof(double));
}
...
void evaluatePowerLawParams(double PTvent, double PTsol,
                            double value_sol, double value_vent,
                            double &k1, double &k2)
{
...
void simulationInitialize(Sciara* sciara)
{
    ...
    sciara->simulation->effusion_duration =
        sciara->simulation->emission_time * maximum_number_of_emissions;
...
// coordinate relative del vicinato di Moore
...
void init(Sciara*& sciara)
{
    ...
    sciara->substates  = new Substates; // Substates allocation is done when the configuration is loaded
...
void makeBorder(Sciara *sciara)
{
    ...
    // the rest
    for (int ii = 1; ii < sciara->domain->rows - 1; ii++)
        for (int jj = 1; jj < sciara->domain->cols - 1; jj++)
            if (calGetMatrixElement(sciara->substates->Sz, sciara->domain->cols, ii, jj) >= 0) {
                for (int k = 1; k < MOORE_NEIGHBORS; k++)
                    if (calGetMatrixElement(sciara->substates->Sz, sciara->domain->cols,
                                            ii + sciara->X->Xi[k], jj + sciara->X->Xj[k]) < 0)
                    {
                        calSetMatrixElement(sciara->substates->Mb, sciara->domain->cols, ii, jj, true);
                        break;
                    }
            }
}
